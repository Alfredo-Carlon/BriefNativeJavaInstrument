...
StaticClassTagger::StaticClassTagger(int32_t startTag)
{
               ...
               const char *className)
{
    ...
    if(!strcmp(className, "null"))
        _canTag = false;
    if(!strncmp(className, "sun/reflect/Generated",
                strlen("sun/reflect/Generated"))){
        _canTag = false;
    }
}
...
void StaticClassTagger::insertReportToMethods(const char *methodsName)
{
    ...
    for(unsigned int i=0; i != nMethods; i++){
        ...
        if(_bytecodeParser ->methodNameEquals(i, methodsName)){
            //_bytecodeParser -> add_aload_n(i, _startByteOffset, 0);
...
{
    try {
        std::string cname = _classNames ->at(tag);
        return cname;
